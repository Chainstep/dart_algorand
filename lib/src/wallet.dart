...
class Wallet {
  ...
  static Future<Wallet> init(
    ...
    if (wallet.id == null) {
      final w = await (kmdClient.createWallet(
          ...
          masterDerivKey: mdk) as FutureOr<APIV1Wallet>);
      wallet.id = w.id;
    }
  ...
  Future<String> getMnemonic() async {
    final mdk = await (exportMasterDerivationKey() as FutureOr<String>);
  ...
  Future<String?> exportKey(String? address) async {
    return await (kmdClient!.exportKey(
        handle: await handle, password: walletPassword, address: address) as FutureOr<String?>);
  }
